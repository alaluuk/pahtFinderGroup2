...
class StructureTemplateEditModal extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleSubmit(e) {
    ...
    return new Promise((resolve, reject) => {
      ...
      mutation($id: ID!, $title: String!, $typeId: ID!, $uValue: Float!, $price: Float, $manufacturer: String, $serialNumber: String, $productionYear: Int) {
    ...
    `, this.state.values)
        .then(data => {
          if(this.props.onEdited) this.props.onEdited(data.updateStructureTemplate);
          AppToaster.show({ icon: "tick", intent: Intent.SUCCESS, message: "Saved changes to \""+data.updateStructureTemplate.title+"\"!" });
          resolve(data);
        })
        .catch(err => {
  ...
  render() {
    return (
      <Dialog
        className="StructureTemplateEditModal Modal"
        icon="layers"
        onOpening={this.reset}
        onClose={this.props.onClose || undefined}
        title="Edit Structure Template"
        {...this.state}
      >
...
export default StructureTemplateEditModal;